...
void all_data_update(void)
{
    ...
    mcu_dp_enum_update(DPID_MOVE_DIR,0); //枚举型数据上报;
    mcu_dp_bool_update(DPID_CHILD_LOCK,0); //BOOL型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_move_dir_handle
功能描述 : 针对DPID_MOVE_DIR的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_move_dir_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为ENUM
    unsigned char ret;
    unsigned char move_dir;
    
    move_dir = mcu_get_dp_download_enum(value,length);
    switch(move_dir) {
        case 0:
        break;
    ...
    ret = mcu_dp_enum_update(DPID_MOVE_DIR, move_dir);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        case DPID_MOVE_DIR:
            //移动方向处理函数
            ret = dp_download_move_dir_handle(value,length);
        break;
